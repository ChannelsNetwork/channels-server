...
export class CardManager implements Initializable, NotificationHandler, CardHandler, RestServer {
  ...
  private async handlePostCard(request: Request, response: Response): Promise<void> {
    try {
      ...
      const card = await this.postCard(request, user, requestBody.detailsObject, requestBody.detailsObject.address);
  ...
  async postCard(request: Request, user: UserRecord, details: PostCardDetails, byAddress: string): Promise<CardRecord> {
    // if (!details.text) {
