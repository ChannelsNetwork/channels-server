...
export class RootPageManager implements Initializable {
  ...
  async initialize(urlManager: UrlManager): Promise<void> {
    ...
    if (!this.templatesLoaded) {
      ...
      let globalJsContent = "<script>\nwindow.googleAnalyticsId = \"" + gaId + "\";\n" + fs.readFileSync(path.join(__dirname, `../${publicFolder}/scripts/global.js`), 'utf8') + "\n</script>";
      globalJsContent += "\n<script>\n" + fs.readFileSync(path.join(__dirname, `../${publicFolder}/scripts/intersection-observer.js`), 'utf8') + "\n</script>";
  ...
  async handlePage(type: string, request: Request, response: Response, card?: CardRecord, channnel?: ChannelRecord, author?: UserRecord): Promise<void> {
    ...
    const metadata = {
      title: "Channels",
      description: "Channels is pioneering a new way for content creators to get paid. A simple micro payment site for content. Publish on Channels and earn money for every view.",
      url: ogUrl,
