...
class ChannelsNetworkWebClient {
  ...
  private async setupExpress(): Promise<void> {
    ...
    this.app.use('/d/terms.html', express.static(path.join(__dirname, '../' + publicPath + "/terms.html"), { maxAge: 1000 * 60 }));
    this.app.use('/whitepaper.pdf', express.static(path.join(__dirname, '../' + publicPath + "/whitepaper.pdf"), { maxAge: 1000 * 60 * 15 }));
    this.app.use('/manifest.json', express.static(path.join(__dirname, '../' + publicPath + "/manifest.json"), { maxAge: 0 }));
    ...
    this.app.use('/v' + SERVER_VERSION, express.static(path.join(__dirname, '../' + publicPath), { maxAge: 1000 * 60 * 60 * 24 }));
